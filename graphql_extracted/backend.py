...
OPERATIONS_DIR = BASE_DIR / "clean_graphql" / "queries" / "query"
...
def operation_path(operation_name: str) -> Path:
    matches = list(OPERATIONS_DIR.glob(f"{operation_name}.graphql"))
    if len(matches) != 1:
        raise RuntimeError(f"operation {operation_name!r} not found uniquely")
    return matches[0]
...
def parse_operation_response(operation_name: str, status: int, body: dict[str, Any]) -> dict[str, Any]:
    ...
    entity = data.get("movie") or data.get("film") or {}
    ...
    with ThreadPoolExecutor(max_workers=len(operations)) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if key == "base":
                    raise
                logger.warning("Optional GraphQL operation %s failed", key, exc_info=True)
                responses[key] = {}
...
async def fetch_film_async(film_id: int, session: ClientSession) -> FilmResponse:
    ...
    return build_film_response(film_id, dict(results))
...
def build_film_response(film_id: int, responses: dict[str, dict[str, Any]]) -> FilmResponse:
    ...
    kp_rating = base_rating.get("kinopoisk") or {}
    ...
    planned_rating = base_rating.get("plannedToWatch") or {}
    ...
    return FilmResponse(
        ...
        year=base.get("productionYear") or general.get("kpYear") or general.get("productionYear"),
        film_length=base.get("duration") or general.get("kpDuration"),
        is_tickets_available=bool(ticket_option.get("purchasable")),
        production_status=base.get("productionStatus"),
        ...
        start_year=((base.get("releaseYears") or {}).get("start")),
        end_year=((base.get("releaseYears") or {}).get("end")),
        cover_url=image_url(
        ...
        serial=typename in {"Series", "TvSeries", "MiniSeries"},
        short_film=typename == "ShortFilm",
        completed=base.get("productionStatus") == "COMPLETED",
